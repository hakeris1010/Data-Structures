...
extern "C" AVLTree<TYPE>* getInternalsFromTree(const C_AVLTree* const tree)
{
    if(!tree)
    {
...
extern "C" void avl_initTree(C_AVLTree* tree) //initialize the tree - always call first!
{
    if(!tree) return;
...
extern "C" void avl_setFreeOnDestroy(C_AVLTree* tree, char val)
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setFreeOnDestroy(val);
}

extern "C" char avl_getFreeOnDestroy(C_AVLTree* tree)
{
    if((!tree ? 1 : !tree->internals) || errCode) return 0;
    return getInternalsFromTree(tree)->getFreeOnDestroy();
}

extern "C" void avl_setElemDestructorCallback(C_AVLTree* tree, void (*valDest)(TYPE*))
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setElemDestructor(valDest);
}

extern "C" void avl_setEvaluatorCallback(C_AVLTree* tree, char (*elemEvaluator)(TYPE, TYPE))
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setElemEvaluator(elemEvaluator);
}

extern "C" void avl_setShowerCallback(C_AVLTree* tree, const char* (*eShower)(TYPE))
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setElemShower(eShower);
}

extern "C" void avl_clearTree_setCallback(C_AVLTree* tree, void (*valDest)(TYPE*), char destroyObject)
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setElemDestructor(valDest);
...
extern "C" void avl_deleteElement_setCallback( C_AVLTree* tree, const TYPE val, void (*valDest)( TYPE* ) )
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->setElemDestructor(valDest);
...
extern "C" void avl_deleteElement( C_AVLTree* tree, const TYPE val )
{
    if((!tree ? 1 : !tree->internals) || errCode) return;
    getInternalsFromTree(tree)->deleteElement(val);
}

extern "C" char avl_findElement_setCallback( C_AVLTree* tree, const TYPE val, char (*evalCallbk)(TYPE v1, TYPE v2) )
{
    if((!tree ? 1 : !tree->internals) || errCode) return 0;
...
extern "C" char avl_findElement( const C_AVLTree tree, const TYPE val )
{
...
extern "C" void avl_showTree( const C_AVLTree tree, char dataShowmode, char pointerShowmode, char branchShowmode )
{
...
extern "C" int getLastErrorCode(char printInfo)
{
    if(printInfo)
    {
        if(errCode == NoError)
